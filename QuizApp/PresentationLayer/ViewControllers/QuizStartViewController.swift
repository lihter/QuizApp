...
class QuizStartViewController: UIViewController {
    ...
    private let containerFromBottomPortraitOffsetMul: CGFloat = 0.2403
    ...
    private func addConstraints() {
        ...
        landscapeConstraints.append(contentsOf:
            container.snp.prepareConstraints{
                $0.top.equalTo(view).offset(width * containerFromTopOffsetMulLandscape)
                $0.bottom.equalTo(view).offset(-1 * width * containerFromBottomOffsetMulLandscape)
            })
        
        portraitConstraints.append(contentsOf:
            container.snp.prepareConstraints{
                $0.top.equalTo(view).offset(height * containerFromTopOffsetMul)
                $0.bottom.equalTo(view).offset(-1 * width * containerFromBottomOffsetMulLandscape)
            })
    ...
    @objc private func startQuiz() {
        coordinator.resetQuizTrackingVariables()
